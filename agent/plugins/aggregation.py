...
class AggregationPlugin(BasePlugin):
    ...
    def process(self, metrics_request: monitoring_pb2.MetricsRequest) -> Optional[monitoring_pb2.MetricsRequest]:
        ...
        if len(self.history) >= self.window_size:
            ...
            metrics_request.metadata["cpu_avg"] = f"{aggregated.get('cpu_percent_avg', 0):.1f}"
            metrics_request.metadata["cpu_min"] = f"{aggregated.get('cpu_percent_min', 0):.1f}"
            metrics_request.metadata["cpu_max"] = f"{aggregated.get('cpu_percent_max', 0):.1f}"
            metrics_request.metadata["mem_avg"] = f"{aggregated.get('memory_percent_avg', 0):.1f}"
            metrics_request.metadata["mem_min"] = f"{aggregated.get('memory_percent_min', 0):.1f}"
            metrics_request.metadata["mem_max"] = f"{aggregated.get('memory_percent_max', 0):.1f}"
            
            # Clear history for next window
            self.history = []
            
            print(f"[AggregationPlugin] ✓ Aggregated window completed (samples={self.send_count}, " +

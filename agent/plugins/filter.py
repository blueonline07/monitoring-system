...
class FilterPlugin(BasePlugin):
    ...
    def initialize(self, config: Optional[Dict[str, Any]] = None):
        ...
        print(f"[FilterPlugin] initialized")
    ...
    def _is_idle(self, metrics: monitoring_pb2.SystemMetrics) -> bool:
        ...
        """
        return (
            metrics.cpu_percent < 10.0 and
            metrics.disk_read_mb < 1.0 and
            metrics.disk_write_mb < 1.0 and
            metrics.net_in_mb < 1.0 and
    ...
    def run(self, metrics_request: monitoring_pb2.MetricsRequest) -> Optional[monitoring_pb2.MetricsRequest]:
        ...
        if not self.send_idle and self._is_idle(metrics):
            ...
            print(f"[FilterPlugin] Filtered (idle system)")
            return None
        
        # Check CPU threshold
        if metrics.cpu_percent < self.min_cpu:
            self.filtered_count += 1
            print(f"[FilterPlugin] Filtered (CPU {metrics.cpu_percent:.1f}% < {self.min_cpu}%)")
            return None
        
        # Check memory threshold
        if metrics.memory_percent < self.min_memory:
            self.filtered_count += 1
            print(f"[FilterPlugin] Filtered (Memory {metrics.memory_percent:.1f}% < {self.min_memory}%)")
            return None
    ...
    def finalize(self):
        ...
        print(f"[FilterPlugin] finalized")
